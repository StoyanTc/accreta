...
fn fun_name() {
    unsafe {
        ...
        let status =
            accreta_engine_ingest(engine, t0 + 60_000, &v2 as *const _, 1, dims.as_ptr(), 1);
        ...
        assert_eq!(sum_val.value.f64, 20.0);
        ...
        let status =
            accreta_engine_bucket(engine, AccretaBucketLevel::Hour, hour_start, &mut bucket);
