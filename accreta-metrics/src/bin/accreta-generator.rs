...
const REGIONS: &[&str] = &["eu-west", "eu-central", "us-east", "us-west", "ap-southeast"];
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            password: env::var("ACCRETA_METRICS_PASSWORD").unwrap_or_else(|_| DEFAULT_PASSWORD.into()),
    ...
    fn parse() -> Result<Self, String> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--events-per-second" => {
                    cfg.events_per_second = next_value(&mut args, "--events-per-second")?.parse().map_err(|_| "invalid --events-per-second".to_string())?;
                }
                "--interval-ms" => {
                    let ms: u64 = next_value(&mut args, "--interval-ms")?.parse().map_err(|_| "invalid --interval-ms".to_string())?;
                ...
                "--time-scale" => cfg.time_scale = next_value(&mut args, "--time-scale")?.parse().map_err(|_| "invalid --time-scale".to_string())?,
                "--seed" => cfg.seed = next_value(&mut args, "--seed")?.parse().map_err(|_| "invalid --seed".to_string())?,
                "--scenario" => cfg.scenario = Scenario::parse(&next_value(&mut args, "--scenario")?)?,
                "--batch-size" => cfg.batch_size = next_value(&mut args, "--batch-size")?.parse().map_err(|_| "invalid --batch-size".to_string())?,
                "--help" | "-h" => {
                    print_help();
                    std::process::exit(0);
                }
...
impl Generator {
    fn new(seed: u64, scenario: Scenario) -> Self {
        Self { rng: Rng::new(seed), simulated_now: Utc::now(), scenario }
    }

    fn advance(&mut self, real_elapsed: Duration, mode: Mode, scale: f64) {
        let seconds = real_elapsed.as_secs_f64() * match mode { Mode::Realtime => 1.0, Mode::Accelerated => scale };
    ...
    fn sample(&mut self) -> Value {
        ...
        let service_factor = match service { "payments" => 1.20, "search" => 0.90, "auth" => 0.75, _ => 1.0 };
        let requests = (traffic * service_factor * (1.0 + 0.20 * self.rng.normalish())).max(0.05) * 100.0;

        let base_latency = match endpoint { "/search" => 70.0, "/checkout" => 95.0, "/orders" => 45.0, _ => 25.0 };
        let region_latency = match region { "ap-southeast" => 18.0, "us-west" => 8.0, "eu-central" => 5.0, _ => 0.0 };
        let latency = (base_latency + region_latency + 10.0 * self.rng.normalish()) * incident.latency;

        let error_probability = (match endpoint { "/checkout" => 0.018, "/orders" => 0.012, _ => 0.008 } * incident.errors).min(0.35);
        let error = self.rng.unit() < error_probability;
        let status = if error {
            if self.rng.unit() < 0.70 { "500" } else { "400" }
        } else if self.rng.unit() < 0.015 { "404" } else { "200" };
    ...
    fn incident_factor(&self) -> IncidentFactor {
        ...
        if !active { return IncidentFactor::default(); }
        match self.scenario {
            Scenario::Normal => IncidentFactor::default(),
            Scenario::TrafficSpike => IncidentFactor { traffic: 4.0, ..Default::default() },
            Scenario::LatencySpike => IncidentFactor { latency: 3.0, ..Default::default() },
            Scenario::ErrorSpike => IncidentFactor { errors: 10.0, ..Default::default() },
            Scenario::Mixed => IncidentFactor { traffic: 2.5, latency: 2.2, errors: 7.0 },
        }
    }
}

#[derive(Debug, Clone, Copy)]
struct IncidentFactor { traffic: f64, latency: f64, errors: f64 }
impl Default for IncidentFactor {
    fn default() -> Self { Self { traffic: 1.0, latency: 1.0, errors: 1.0 } }
}

async fn request(method: &str, url: &str, token: Option<&str>, body: Option<&str>) -> io::Result<HttpResponse> {
    let without_scheme = url.strip_prefix("http://").ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "only http:// URLs are supported"))?;
    ...
    let status = headers.lines().next().and_then(|l| l.split_whitespace().nth(1)).and_then(|s| s.parse().ok()).unwrap_or(0);
...
async fn login(cfg: &Config) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let response = request("POST", &format!("{}/login", cfg.base_url.trim_end_matches('/')), None, Some(&body)).await?;
    if response.status != 200 { return Err(format!("login failed (HTTP {}): {}", response.status, response.body).into()); }
...
async fn create_schema(cfg: &Config, token: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let response = request("POST", &format!("{}/schema", cfg.base_url.trim_end_matches('/')), Some(token), Some(&body)).await?;
...
async fn ingest(cfg: &Config, token: &str, samples: &[Value]) -> Result<usize, Box<dyn std::error::Error>> {
    let body = serde_json::to_string(&json!({"samples": samples}))?;
    let response = request("POST", &format!("{}/schema/ingest", cfg.base_url.trim_end_matches('/')), Some(token), Some(&body)).await?;
    if response.status != 200 { return Err(format!("ingest failed (HTTP {}): {}", response.status, response.body).into()); }
    #[derive(Deserialize)] struct IngestResponse { ingested: usize }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        ...
        let events = ((cfg.events_per_second as f64) * cfg.interval.as_secs_f64()).round().max(1.0) as usize;
        ...
        if !samples.is_empty() { sent += ingest(&cfg, &token, &samples).await?; }
        total += sent;
